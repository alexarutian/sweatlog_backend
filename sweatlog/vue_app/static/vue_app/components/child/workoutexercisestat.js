...
let WorkoutExerciseStat = {
  ...
  methods: {
    ...
    deleteDraggable(item) {
      this.$store.commit("removeFromWorkoutSelectedItemList", { item });
    },
    selectTime(e) {
      this.time_in_seconds = e.target.value;
    },
    saveStats() {
      let exercise = this.$store.getters.getWorkoutSelectedExerciseByIndex(this.index);
      ...
      this.$store.commit("swapWorkoutSelectedItem", {
        blockIndex: 0,
  ...
  computed: {
    statPopulated() {
      ...
      let exercise = this.$store.getters.getWorkoutSelectedExerciseByIndexes(0, this.index);
